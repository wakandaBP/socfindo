<script type="text/javascript">
	...
	$(function(){
		...
		var germpreparelogList = $("#list-germ-prepare-log").DataTable({

			"ajax":{
				"url": hostname + "/api/germ-prepare/loader.germ-prepare-screen.log.php",
				"data":{
					id:<?php echo $page[1];?>
				},
				"type":"POST"
		...
		$("#btnAddScreen").click(function (){
			...
			$.ajax({
				async:false,
				url: hostname + "/api/germ-prepare/germ-prepare.log.count.php",
				data : {id:<?= $page[1]; ?>},
				type : "POST",
			...
			$("#count").html(count);
			$("form")[0].reset();
			$("#form-germ-prepare-screen").modal("show");
		});

		//var workerSelect = $('#screenworker');

		$("#list-germ-prepare-log tbody").on('click','.btn-editLog', function (){
			$("form")[0].reset();
			id = $(this).data("id");
			mode = "update";
			...
			$.ajax({
				//async:false,
				url: hostname + "/api/germ-prepare/load.log.data.germ-prepare.php",
				data : {id:id},
				type : "POST",
				success: function(resp){
		...
		$("#btnSaveLog").click(function(){
			if (mode == "add"){
				action = "add-germ-prepare-screening";
			} else if (mode == "update") {
				action = "update-germ-prepare-screening";
			}
				...
				$.ajax({
					...
					success:function(resp){
						...
						if(parseInt(resp) > 0){
                        	...
                        	$("#form-germ-prepare-screen").modal("hide");
                        	RefreshData("#list-germ-prepare-log",hostname + "/api/germ-prepare/loader.germ-prepare-screen.log.php",({id:<?= $page[1];?>}) );
                        	showRowLastRow(germpreparelogList);
                        }
                        else{
