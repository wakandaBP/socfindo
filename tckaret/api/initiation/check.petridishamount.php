<?php 
	...
	if (isset($_POST['amount']) AND isset($_POST['media'])){
		...
		$cekmedia = new Database("SELECT a.id as idpembuatan, b.id as idmedia FROM karet_media_pembuatan a 
								JOIN karet_media b ON b.id = a.idmedia WHERE a.idmedia = ? AND b.isactive = ?"
								,array($_POST['media'],1));
		...
		foreach ($cekmedia::$result as $key => $item) {
			...
			$pemasukan = new Database("SELECT jumlah FROM karet_media_pembuatan WHERE id = ? AND isactive = ?",array($idpembuatan,1));
